...
const calculateRoundUps = (transactions) => {
  return transactions.map(tx => ({
    ...
    category: tx.description || tx.category,
...
export const apiService = {
  ...
  async getTransactions(useMockData = false, customerName = '') {
    ...
    if (customerData.data && customerData.data.length > 0) {
      ...
      // Reverse the array to read from end to beginning
      return { data: transactionsWithRoundUps.reverse() };
    }
    return { data: [] };
  },
  ...
  async getAIInsights(useMockData = false, spendingData) {
    ...
    try {
      ...
      // Fallback to mock data if AI endpoint doesn't exist
  ...
  async generateMoreInsights(useMockData = false, context) {
    ...
    try {
      const response = await apiClient.post('/ai/more-insights', { context });
      return response;
    } catch (error) {
      // Fallback to mock data if AI endpoint doesn't exist
      console.warn('AI more insights endpoint not available, using mock data');
      await mockDelay(1000);
      return { 
        data: [
          {
